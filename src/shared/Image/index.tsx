    ...
    const PlaceholderImage = useMemo(() => {
        ...
        return (
            <ImageComponent
                ...
                resizeMode="contain"
                {...placeholderImageProps}
                style={[imageStyle, StyleSheet.absoluteFillObject, placeHolderBg, placeholderImageProps?.style]}
            />
        );
    }, [ImageComponent, imageStyle, placeholderImage, placeholderImageProps]);

    return (
        <Container style={containerImageStyle}>
            {CachedImageMemoized}
            {((typeof source !== 'number' && !source?.uri) || isError) && PlaceholderImage}
        </Container>
    );
};
